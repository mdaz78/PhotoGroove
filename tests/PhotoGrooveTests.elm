...
firstImageGetsSelected =
    fuzz urlFuzzer "When the photos load, the first one gets selected" <|
        \urls ->
            let
                photos =
...
-- slideHueSetsHue =
--     fuzz int "Slide Hue tests the Hue" <|
--         \amount ->
--             initialModel
...
clickThumbnail : Test
clickThumbnail =
    fuzz3 urlFuzzer string urlFuzzer "clicking thumbnail selects it" <|
